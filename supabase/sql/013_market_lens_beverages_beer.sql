-- Adds Beverage/Beer coverage to the legacy Market Lens fallback functions.
-- bbs_market_subsector/bbs_market_category previously had no branch at all for
-- "Bebidas" (sector Industria) — any row without persisted market_* columns fell
-- through to the generic Industria -> Manufacturing / General Industry branch.
-- This mirrors the split newly added to warehouse/taxonomy/{sector_subsector_
-- category_v1,market_lens_rules_v1}.json: Cervezas gets its own market_category
-- ("Beer"), separate from Bebidas alcohólicas ("Alcoholic Drinks") and Bebidas no
-- alcohólicas ("Non-Alcoholic Drinks"), all under market_subsector "Beverage".
--
-- Only affects rows lacking persisted market_sector/subsector/category (legacy
-- rows, or callers that bypass the persisted columns) — see
-- 009_fix_tracking_series_use_persisted_market_columns.sql for why persisted
-- columns are preferred whenever present.
