...
language sql
immutable
as create or replace function public.bbs_norm_taxon(v text)
returns text
language sql
immutable
as $$
  select regexp_replace(translate(lower(coalesce(v, '')), '???????', 'aeiouun'), '\\s+', ' ', 'g');
$$;
...
as $$
  select case
    when public.bbs_norm_taxon(p_category) in ('farmacias','hospitales','clinicas','laboratorios') then 'Health & Wellness'
...
as $$
  select case
    ...
    when public.bbs_norm_taxon(p_category) in ('hospitales','clinicas','laboratorios') then 'Healthcare Services'
...
as $$
  select case
    ...
    when public.bbs_norm_taxon(p_category) = 'clinicas' then 'Clinics'
...
begin
  ...
  with base_years as (
    ...
    where (v_sector is null or
      (case when v_taxonomy_view='market' then public.bbs_market_sector(jm.sector, jm.subsector, jm.category) else coalesce(jm.sector, 'Unassigned') end) = v_sector)
      and (v_subsector is null or
      (case when v_taxonomy_view='market' then public.bbs_market_subsector(jm.sector, jm.subsector, jm.category) else coalesce(jm.subsector, 'Unassigned') end) = v_subsector)
      and (v_category is null or
      (case when v_taxonomy_view='market' then public.bbs_market_category(jm.sector, jm.subsector, jm.category) else coalesce(jm.category, 'Unassigned') end) = v_category)
  ...
  return (
    with journey_base as (
      select
        ...
        case
          when v_taxonomy_view='market' then public.bbs_market_sector(jm.sector, jm.subsector, jm.category)
          else coalesce(jm.sector, 'Unassigned')
        end as tx_sector,
        case
          when v_taxonomy_view='market' then public.bbs_market_subsector(jm.sector, jm.subsector, jm.category)
          else coalesce(jm.subsector, 'Unassigned')
        end as tx_subsector,
        case
          when v_taxonomy_view='market' then public.bbs_market_category(jm.sector, jm.subsector, jm.category)
          else coalesce(jm.category, 'Unassigned')
        end as tx_category,
        case
          when v_breakdown='sector' then
            case when v_taxonomy_view='market' then public.bbs_market_sector(jm.sector, jm.subsector, jm.category) else coalesce(jm.sector, 'Unassigned') end
          when v_breakdown='subsector' then
            case when v_taxonomy_view='market' then public.bbs_market_subsector(jm.sector, jm.subsector, jm.category) else coalesce(jm.subsector, 'Unassigned') end
          when v_breakdown='category' then
            case when v_taxonomy_view='market' then public.bbs_market_category(jm.sector, jm.subsector, jm.category) else coalesce(jm.category, 'Unassigned') end
    ...
    touchpoint_base as (
      select
        ...
        case
          when v_breakdown='sector' then
            case when v_taxonomy_view='market' then public.bbs_market_sector(tm.sector, tm.subsector, tm.category) else coalesce(tm.sector, 'Unassigned') end
          when v_breakdown='subsector' then
            case when v_taxonomy_view='market' then public.bbs_market_subsector(tm.sector, tm.subsector, tm.category) else coalesce(tm.subsector, 'Unassigned') end
          when v_breakdown='category' then
            case when v_taxonomy_view='market' then public.bbs_market_category(tm.sector, tm.subsector, tm.category) else coalesce(tm.category, 'Unassigned') end
      ...
      where (v_sector is null or
          (case when v_taxonomy_view='market' then public.bbs_market_sector(tm.sector, tm.subsector, tm.category) else coalesce(tm.sector, 'Unassigned') end) = v_sector)
        and (v_subsector is null or
          (case when v_taxonomy_view='market' then public.bbs_market_subsector(tm.sector, tm.subsector, tm.category) else coalesce(tm.subsector, 'Unassigned') end) = v_subsector)
        and (v_category is null or
          (case when v_taxonomy_view='market' then public.bbs_market_category(tm.sector, tm.subsector, tm.category) else coalesce(tm.category, 'Unassigned') end) = v_category)
