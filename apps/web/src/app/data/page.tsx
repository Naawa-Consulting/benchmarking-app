...
export default function RulesStudioPage() {
  ...
  const canMutate = authz?.can_mutate ?? !authEnabled;
  ...
  useEffect(() => {
    const loadStudies = async () => {
      const result = await getDataStudiesDetailed();
    ...
    loadStudies();
    window.addEventListener("bbs:data-studies-changed", handleRefreshStudies);
    return () => {
      window.removeEventListener("bbs:data-studies-changed", handleRefreshStudies);
    };
  }, []);
    ...
    <main className="space-y-6">
      <section className="main-surface rounded-3xl p-6">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <h2 className="text-3xl font-semibold">Data Studio</h2>
            <p className="mt-2 text-slate">
              Build mapping rules from questions (stage + brand extraction).
            </p>
            {authz ? (
              <p className="mt-2 text-xs text-slate">
                Role: <span className="font-semibold text-ink">{authz.role}</span> · Mutating actions:{" "}
                <span className={canMutate ? "text-emerald-700" : "text-amber-700"}>
                  {canMutate ? "enabled" : "disabled"}
                </span>{" "}
                · Permissions:{" "}
                <span className="font-medium text-ink">
                  {authz.permissions.length ? authz.permissions.join(", ") : "none"}
                </span>
              </p>
            ) : null}
          </div>
          ...
          <div className="main-surface rounded-3xl p-6 space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              <button
                ...
                type="button"
                disabled={!canMutate || studyVariablesState === "loading"}
              >
                Save
              </button>
              <button
          ...
          <div className="main-surface rounded-3xl p-6 space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              <button
                ...
                type="button"
                disabled={!canMutate || taxonomyState === "loading"}
              >
                Save classification
              </button>
              <button
              ...
              <div className="main-surface rounded-3xl p-6 space-y-4">
                <div className="flex flex-wrap gap-3">
                  ...
                  <button
                    ...
                    type="button"
                    disabled={!canMutate || !selectedStudyId}
                  >
